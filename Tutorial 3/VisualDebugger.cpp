...
namespace VisualDebugger
{
	...
	void UpdateHUD(int screen) { //custom function for updating a HUD screen content when things happen e.g goal scored
		switch (screen) {
		case BASE:
			hud.Clear(BASE); //clear screen
			//re-add lines

			hud.AddLine(BASE, "");
			hud.AddLine(BASE, "Red Team Score: " + std::to_string(scene->redTeamScore));
			hud.AddLine(BASE, "");
			hud.AddLine(BASE, "Blue Team Score: " + std::to_string(scene->blueTeamScore));
		}
	}
	...
	{
		//initialise HUD
		//add a base screen showing scores
		hud.AddLine(BASE, "");
		hud.AddLine(BASE, "Red Team Score: " + std::to_string(scene->redTeamScore));
		hud.AddLine(BASE, "");
		hud.AddLine(BASE, "Blue Team Score: " + std::to_string(scene->blueTeamScore));
		...
		hud.AddLine(HELP, "    F5 - help on/off/scorboard");
	...
	public:
		float renderTime, updateTime, fps;
		int actorCount;
		...
		void output() {
			if (timer.getChronoTime() > 1) { //only output every 1s - if this changes, change fps counter
				cout << endl;
				cout << "Render time: " << renderTime << endl;
				cout << "Update time: " << renderTime << endl;
				cout << "FPS: " << fps << endl;
	...
	void RenderScene()
	{
		...
		if (hud_show)
		{
			...
			hud.ActiveScreen(BASE);

	
		//update HUD with variable changes
		UpdateHUD(BASE);
		...
		//get render time
		profile.renderTime = timer.getChronoTime();
		timer.resetChronoTimer();
		...
		//get timer for update time
		profile.updateTime = timer.getChronoTime();

		//update actor numbers
		profile.actorCount = scene->GetAllActors().size();
	...
	void UserKeyPress(int key)
	{
		...
		//implement your own
		case 'T': 
			scene->catapultForward();
			break;
		case 'G': 
			scene->catapultBack();
			break;
		case 'F': 
			scene->catapultLeftTurn();
			break;
		case 'H':
			scene->catapultRightTurn();
			break;
	...
	void UserKeyRelease(int key)
	{
		...
		{
		//implement your own
		case 'C': //catapultTrigger
			//pass key into custom handler function
			scene->catapultTriggerHandler();
			break;
