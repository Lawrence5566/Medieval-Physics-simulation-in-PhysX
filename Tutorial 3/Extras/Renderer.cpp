...
namespace VisualDebugger
{
	...
	namespace Renderer
	{
		...
		void DrawInvisibleBox(const PxGeometryHolder& geometry)
		{
			PxVec3 half_size = geometry.box().halfExtents;
			glScalef(half_size.x, half_size.y, half_size.z);
			//glutSolidCube(2.f);
			...
			for(PxU32 i=0;i<numActors;i++)
			{
				...
				else if (actors[i]->isRigidActor())
				{
					...
					for(PxU32 j = 0; j < shapes.size(); j++)
					{
						...
						//check if a invis object (one of the goal collision boxes)
						if (actors[i]->getName() == redGoalName || actors[i]->getName() == blueGoalName) {
							DrawInvisibleBox(h);
						}
						else {
							RenderGeometry(h);
